...
class MazeNode(Node):
    ...
    def main(self):
        ...
        # WE HAVE SEEN BLUE!
        if self.camera.size > 50: # maybe check that 50 is good
            # run towards it
        ...
        elif (self.lidar.left < self.l_let and self.lidar.front < self.f_let): #left and front are blocked
            angular_z = -self.turning_speed #move right

        elif (self.lidar.left > self.l_let and self.lidar.front > self.f_let): #left and front are unblocked
            angular_z = self.turning_speed # move left
            linear_y = self.forward_speed

        elif (self.lidar.left > self.l_let and self.lidar.front < self.f_let): #left is unblocked, front is blocked
            angular_z = self.turning_speed # move left
