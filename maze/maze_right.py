...
class MazeNode(Node):
    def __init__(self):
        ...
        # Variables
        self.r_let = .25  # how close the right wall needs to be to register as "close"
        self.f_let = .3   # how close the front wall needs to be to register as "close"
        self.r_let_CONST = self.r_let
        self.lidar = Lidar()
        self.camera = Camera()
    ...
    def main(self):
        ...
        # WE HAVE SEEN BLUE!
        if self.camera.size > 50:
            # run towards it
            linear_x = self.forward_speed

            # also strafe if we're off center
            if self.camera.x > 0 and self.camera.x < self.mid_cam * 2:
                lateralError = self.camera.x-self.mid_cam
                if(lateralError < 0):
                    #to strafe left slightly
                    linear_y = (1 - abs(lateralError) * self.kP) * self.forward_speed * .5
                else:
                    #to strafe right slightly
                    linear_y = -(1 - abs(lateralError) * self.kP) * self.forward_speed * .5

        elif (self.lidar.right < self.r_let and self.lidar.front < self.f_let): #right and front are blocked
            angular_z = self.turning_speed #move left

        elif (self.lidar.right > self.r_let and self.lidar.front > self.f_let): #right and front are unblocked
            angular_z = -self.turning_speed # move right
            linear_y = -self.forward_speed

        elif (self.lidar.right > self.r_let and self.lidar.front < self.f_let): #right is unblocked, front is blocked
            angular_z = -self.turning_speed # move right
            linear_y = -self.forward_speed # should double check if this is a good idea

        else:
            linear_x = self.forward_speed
            # if self.lidar.left < .25 - .1:
            #     angular_z = -self.turning_speed

            if self.lidar.right > .12:
                linear_y = -self.forward_speed/2
